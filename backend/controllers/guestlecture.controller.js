...
// controllers/guestLecture.controller.js
import { Guest } from "../models/guestlecture.model.js";
import cloudinary from "../utils/cloudinary.js";
import getDataUri from "../utils/datauri.js";
...
export const guestlecture = async (req, res) => {
  try {
    ...
    if (!fullname || !email || !phonenumber || !topic) {
      return res.status(400).json({ message: "Some data is missing", success: false });
    }
    ...
    return res.status(201).json({ message: "Guest lecture created successfully", success: true });
  } catch (error) {
    console.error("Guest Lecture Error:", error);
...
export const getAllLecture = async (req, res) => {
  try {
    ...
    if (!data) {
      return res.status(404).json({ message: "No lecture scheduled", success: false });
    }
