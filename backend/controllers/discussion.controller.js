...
export const discussionPost = async (req, res) => {
  try {
    const { content, comment, email } = req.body;
    if (!content) {
      return res.status(400).json({ message: "Content is missing.", success: false });
    }

    const newDiscussion = await discuss.create({ content, comment, email });
    return res.status(201).json({ message: "Discussion created successfully.", success: true, data: newDiscussion });
  } catch (error) {
...
export const createDiscussionPost = async (req, res) => {
  try {
    ...
    if (!thought || !user) {
      return res.status(400).json({ message: "Thought and user are required.", success: false });
    }

    const newDiscussion = new discuss({ content: thought, user });
    await newDiscussion.save();
    res.status(201).json({ success: true, discussion: newDiscussion });
  } catch (error) {
...
export const getallpost = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: "Error fetching discussions" });
  }
};
...
export const discussionDelete = async (req, res) => {
  try {
    ...
    if (existingDiscussion.email.toString() !== userId) {
      return res.status(403).json({ message: "Unauthorized to delete this discussion.", success: false });
    }
